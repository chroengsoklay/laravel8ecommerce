...
1                        <a href="#category_{{$home_category->id}}" class="tab-control-item {{$key == 0 ? 'active':''}}">{{$home_category->name}}</a>
                        @endforeach
                    </div>
                    <div class="tab-contents">
                        @foreach($home_categories as $key => $home_category)
                        <div class="tab-content-item {{$key == 0 ? 'active':''}}" id="category_{{$home_category->id}}">
                            <div class="wrap-products slide-carousel owl-carousel style-nav-1 equal-container" data-items="5" data-loop="false" data-nav="true" data-dots="false" data-responsive='{"0":{"items":"1"},"480":{"items":"2"},"768":{"items":"3"},"992":{"items":"4"},"1200":{"items":"5"}}' >
                                    @php
                                    $c_products = DB::table('products')->where('category_id', $home_category->id)->get()->take($no_of_products);
                                    @endphp
